# AGENTS.md — ERP Universal v4.0
> Instrucciones de sesión para Codex. Este archivo se carga automáticamente al inicio de cada conversación.
...
|-----|--------------|-----------------| 
| **Owner / Project Manager** | Carlos García (`gfcar`) | Dirección del producto, validación de negocio, aprobaciones finales |
| **Owner / Colaborador técnico** | Fabrizio López (`Fabri` / `Fabrilp2002`) | Co-propiedad del producto, colaboración técnica y aprobación de cambios compartidos |
| **Arquitecto** | `Gemini` | Estructura, decisiones de alto nivel, ecosistema, algoritmos |
| **Desarrollador** | `Codex` | Código limpio, UI/UX, backend, implementación |
...
Cada vez que Codex termina trabajo en una sesión, debe agregar una fila en `BITACORA_COLABORATIVA.md` con el formato:

```
| YYYY-MM-DD | Codex | <Resumen técnico breve> | <archivos modificados> | <Estado para la siguiente IA> |
```
...
Todo commit generado por Codex debe incluir:
...
Antes de escribir código nuevo, Codex **debe leer la última entrada de la bitácora**. Si hay trabajo de Gemini pendiente de revisión, auditarlo primero y dejar el resultado ("Aprobado" / "Necesita Ajustes") antes de continuar.
